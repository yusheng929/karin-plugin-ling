...
export const luckylist = karin.command(/^#(查)?(幸运)?字符(列表)?$/, async (e) => {
  ...
  for (const item of items) {
    msg.push([
      ...
      segment.text(`点亮状态: ${item.light_up_info.char_count === item.light_up_info.light_up_char_count ? '已点亮' : item.light_up_info.light_up_char_count + '/' + item.light_up_info.char_count}`)
    ].join('\n'))
  }
