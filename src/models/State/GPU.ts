...
(async function initGetIsGPU () {
  ...
  // 初始化GPU获取
  if (controllers?.find((item: { memoryUsed: any; memoryFree: any; utilizationGpu: any; }) =>
...
export default async function getGPU () {
  ...
  try {
    const { controllers } = await si.graphics()
    const graphics = controllers?.find((item: { memoryUsed: any; memoryFree: any; utilizationGpu: any; }) =>
      item.memoryUsed && item.memoryFree && item.utilizationGpu
    )
    if (!graphics) {
