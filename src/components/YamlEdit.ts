...
const EditAddend = async (e: { reply: (arg0: string, arg1: { at: boolean } | undefined) => any }, Msg1: any, Msg2: any, term: string, value: any, path: any) => {
  try {
...
const EditRemove = async (e: { reply: (arg0: string, arg1: { at: boolean } | undefined) => any }, Msg1: any, Msg2: any, term: string, value: any, path: any) => {
  try {
    const yaml = new YamlEditor(`${CfgPath}/${path}.yaml`)
    const data = yaml.get(term)
    if (!Array.isArray(data)) {
...
const EditTest = async (e: { group_id: any }) => {
  try {
