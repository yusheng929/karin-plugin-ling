import { render } from '../../components/puppeteer.js';
import QQApi from '../../models/api/QQApi.js';
...
export const luckylist = karin.command(/^#(查)?(幸运)?字符(列表)?$/, async (e) => {
    ...
    if (!data)
        return e.reply('❌请稍后再试');
    if (data.retcode !== 0)
        return e.reply('❌获取数据错误');
    const msgs = [];
...
export const luckyword = karin.command(/^#抽(幸运)?字符$/, async (e) => {
    ...
    if (!data)
        return e.reply('❌请稍后再试');
    if (data.retcode === 11004 && (data.msg === 'over svip max times' || data.msg === 'over member max times'))
        return e.reply('❌今日抽取次数已达上限');
    if (data.retcode !== 0)
        return e.reply('❌发送数据错误');
    if (Object.keys(data.data).length === 0)
        return e.reply(segment.image(`file://${dirPath}/resources/luckword/null.png`));
    const item = {
...
export const lucksetting = karin.command(/^#(开启|关闭)(幸运)?字符$/, async (e) => {
    ...
    if (!data)
        return e.reply('❌请稍后再试');
    if (data.retcode === 11111 && (data.retmsg === 'repeat open' || data.retmsg === 'repeat close'))
        return e.reply('❌幸运字符已经处于' + (data.retmsg === 'repeat open' ? '开启' : '关闭') + '状态');
    if (data.retcode !== 0)
        return e.reply('❌发送数据错误');
...
export const luckequip = karin.command(/^#替换(幸运)?字符(\d+)$/, async (e) => {
    ...
    if (!id)
        return e.reply('❌请输入正确的字符ID');
    const data = await new QQApi(e).luckequip(e.groupId, id);
    if (!data)
        return e.reply('❌请稍后再试');
    if (data.retcode === 11106 && data.retmsg === 'word not draw')
        return e.reply('❌未拥有当前id的幸运字符');
    if (data.retcode === 11201 && data.retmsg === 'word same with equiped')
        return e.reply('❌已经装备了当前id的幸运字符');
    if (data.retcode !== 0)
        return e.reply('❌发送数据错误');
    return e.reply('✅替换成功');
