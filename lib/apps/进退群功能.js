...
export const deal_group_apply = karin.accept('request.group_apply', async (e) => {
    ...
    const opts = Array.isArray((Config.getYaml('config', 'other')).DealRequest.Group) ? (Config.getYaml('config', 'other')).DealRequest.Group.find((group) => group[e.group_id])[e.group_id] : (Config.getYaml('config', 'other')).DealRequest.Group;
    if (!opts)
        return false;
    if (opts.accept) {
