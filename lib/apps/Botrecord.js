...
import pb from '../models/protobuf/index.js';
const loginlist = {};
export const violationquery = karin.command(/^#(我的|(机器人|bot))违规记录(查询)?([1-9]\d*)?$/i, async (e) => {
    ...
    if (!code) {
        ...
        return e.reply('获取code失败!', { reply: true });
    ...
    if (result.retcode !== 0 || !result.data) {
        if (loginlist[`${uin}_code`])
            delete loginlist[`${uin}_code`];
    ...
    for (const record of records) {
        const violationinfo = violationlist.find((val) => {
...
async function LightAppGetCode(e, appid) {
    ...
    const payload = await e.bot.sendApi('.send_packet', {
...
// eslint-disable-next-line @stylistic/object-curly-spacing, @stylistic/key-spacing, @stylistic/quotes, @stylistic/comma-spacing
