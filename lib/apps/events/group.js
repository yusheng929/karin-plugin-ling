import { config, karin, logger, redis, segment } from 'node-karin';
import { other } from '../../utils/config.js';
import lodash from 'node-karin/lodash';
import { sendToAllAdmin, sendToFirstAdmin } from '../../utils/common.js';
/** 进群事件 */
export const accept = karin.accept('notice.groupMemberAdd', async (e) => {
    ...
    if (cfg.joinGroup.includes(e.groupId)) {
        ...
        for (let i = 1; i <= 3; i++) {
            ...
            const event = await karin.ctx(e, { time: 180, reply: false }).catch(() => null);
