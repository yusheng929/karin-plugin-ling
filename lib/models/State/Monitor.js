...
export default new class monitor {
    ...
    async init() {
        ...
        // 给有问题的用户关闭定时器
        if (!(Config.getYaml('config', 'state')).statusTask)
            return;
        if ((Config.getYaml('config', 'state')).statusPowerShellStart)
            si.powerShellStart();
        // 初始化数据
        this.getData();
        // 网速
    ...
    async getData() {
        ...
        const { fsStats, networkStats, mem: { active }, currentLoad: { currentLoad } } = data;
    ...
    async getRedisChartData() {
        const data = await redis.get(this.chartDataKey);
        if (data) {
