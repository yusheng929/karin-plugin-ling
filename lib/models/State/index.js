...
export async function getData(e) {
    ...
    const start = Date.now();
    const [visualData, FastFetch, HardDisk, psTest, BotStatusList, style] = await Promise.all(promiseTaskList).then(res => {
        const end = Date.now();
    ...
    const chartData = JSON.stringify(checkIfEmpty(Monitor.chartData, ['echarts_theme', 'cpu', 'ram'])
        ? ''
        : Monitor.chartData);
    // 配置
...
function checkIfEmpty(data, omits = []) {
    const filteredData = _.omit(data, omits);
    return _.every(filteredData, (value) => _.isPlainObject(value) ? checkIfEmpty(value) : _.isEmpty(value));
}
